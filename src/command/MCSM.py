...
class MCSM:
    ...
    def reg(self, msg_data):
        ...
        try:
            re = post.json_headers(config["MCSM"]["url"] + "/api/auth?apikey=" + config["MCSM"]["apikey"], self.headers,
                                   data)
            if bool(re.json()['data']):
                send.group_msg(
                    f"[CQ:at,qq={self.qq}]注册成功\n-------------------------------------\n默认密码:{self.qq}@Ab123456\n"
                    f"请及时进入面板更改密码\n若忘记密码请让管理员帮忙重置",
                    False)
    ...
    def CI(self):
        ...
        return post.json_headers(
            config["MCSM"]["url"] + "/api/instance?apikey=" + config["MCSM"]["apikey"] + "&remote_uuid=" +
            config["MCSM"]["AAI"]["remote_uuid"], self.headers, data).json()

    # 自动分配实例
    def AAI(self, CIData):
        # 查询用户uuid
        uuid = get.get_json(config["MCSM"]["url"] + "/api/auth/search?apikey=" + config["MCSM"][
            "apikey"] + "&userName=" + self.qq + "&page=1&page_size=10000")['data']['data'][0]['uuid']
        # 分配实例数据处理
        data = {
            "uuid": uuid,
                ...
                "instances": [
                    {
                        ...
                        "serviceUuid": config["MCSM"]["AAI"]["remote_uuid"]
                    }
                ]
            }
        }
        put.headers_json(config["MCSM"]["url"] + "/api/auth/?apikey=" + config["MCSM"]["apikey"], self.headers, data)
        MCSM_port = doc.yaml_read("./res/MCSM/port.yaml")
        port = str(MCSM_port['port'])
